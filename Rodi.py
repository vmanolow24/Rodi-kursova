...
def men_page():
    ...
    button = tk.Button(men_frame, text="Add to cart", command=add_to_cart)
    button.pack() 
...
def women_page():
    ...
    label.pack(pady=10)
    tree = ttk.Treeview(women_frame)
    categories = {"-T": "Topwear", "-B": "Bottomwear", "-S": "Shoes", "-R": "Tracksuit"}
    parents = {category: tree.insert("", "end", text=name) for category, name in categories.items()}
    with open("womens.txt", "r") as file:
        for line in file:
            for category, parent in parents.items():
                if category in line:
                    tree.insert(parent, "end", text=line.strip())
    ...
    tree.pack()
    tree.pack(fill=tk.BOTH, expand=True) 
    button = tk.Button(women_frame, text="Add to cart", command=cart_page)
    button.pack() 
    women_frame.grid_rowconfigure(1, weight=1)
    women_frame.grid_columnconfigure(0, weight=1)
def kids_page():
    main_window.withdraw()
    kids_window = tk.Toplevel(main_window)
    kids_window.geometry("900x600")
    ...
    tree.pack()
    tree.pack(fill=tk.BOTH, expand=True) 
    button = tk.Button(kids_frame, text="Add to cart", command=cart_page)
    button.pack() 
    kids_frame.grid_rowconfigure(1, weight=1)
    kids_frame.grid_columnconfigure(0, weight=1)
def nutritions_page():
    main_window.withdraw()
    nutritions_window = tk.Toplevel(main_window)
    nutritions_window.geometry("900x600")
    ...
    # label.place(relx = 0.5, rely = 0.5, anchor = tk.TOP)
    label.pack(pady=10)
    tree = ttk.Treeview(nutritions_frame)
    categories = {"-P": "Protein", "-B": "Bars"}
    parents = {category: tree.insert("", "end", text=name) for category, name in categories.items()}
    ...
    tree.pack()
    tree.pack(fill=tk.BOTH, expand=True) 
    button = tk.Button(nutritions_frame, text="Add to cart", command=cart_page)
    button.pack() 
    nutritions_frame.grid_rowconfigure(1, weight=1)
    nutritions_frame.grid_columnconfigure(0, weight=1)
def accessories_page():
    main_window.withdraw()
    accessories_window = tk.Toplevel(main_window)
    accessories_window.geometry("900x600")
    ...
    # label.place(relx = 0.5, rely = 0.5, anchor = tk.TOP)
    label.pack(pady=10)
    tree = ttk.Treeview(accessories_frame)
    categories = {"-A": "Accessories"}
    parents = {category: tree.insert("", "end", text=name) for category, name in categories.items()}
    with open("accessoaries.txt", "r") as file:
        for line in file:
            for category, parent in parents.items():
                if category in line:
                    tree.insert(parent, "end", text=line.strip())
    ...
    tree.pack()
    tree.pack(fill=tk.BOTH, expand=True) 
    button = tk.Button(accessories_frame, text="Add to cart", command=cart_page)
    button.pack() 
